...
private[netty] class NettyRpcEnv(
  ...
  private[netty] def ask[T: ClassTag](message: RequestMessage, timeout: RpcTimeout): Future[T] = {
    ...
    val remoteAddr = message.receiver.address // 通过 EndpointRef 获取
    ...
    try {
      if (remoteAddr == address) { // 本机
        ...
        val rpcMessage = RpcOutboxMessage(serialize(message), // 远程
          onFailure,
 ...
 * 分发接收到的消息到对应的 Endpoint 上，同时保存 remote client 的 地址
